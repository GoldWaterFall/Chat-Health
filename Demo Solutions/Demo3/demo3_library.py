...
def match(path):
    ...
    id_image = face_recognition.load_image_file(path + 'id_card.jpg')
    id_image = cv.cvtColor(id_image, cv.COLOR_BGR2RGB)
    id_image = cv.resize(id_image, (0, 0), None, 0.5, 0.5)
    ...
    face = face_recognition.face_locations(id_image)[0]
    id_encode = face_recognition.face_encodings(id_image)[0]
    cv.rectangle(id_image, (face[3], face[0]), (face[1], face[2]), (255, 0, 255), 2)

    result1 = face_recognition.compare_faces([ap_encode], ma_encode, 0.4)
    result2 = face_recognition.compare_faces([ap_encode], id_encode, 0.4)
